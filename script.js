// === script.js ===
// REEL C - Console Anni '70 - Versione HTML5
// Autore: StefanoValto64
...
function bootSequence() {
  ...
  const flashInterval = setInterval(() => {
    ...
    if (flashCount >= 10) {
      ...
      applyCRT();
      sounds.paddle();
      setTimeout(showResolutionMenu, 1000);
    }
  }, 500);
}
...
window.addEventListener("keydown", e => {
  ...
  if (e.keyCode === 80 && currentGame && !currentGame.waiting && !currentGame.gameOver && !currentGame.matchOver && !currentGame.paused) {
    currentGame.paused = true;
  } else if (e.keyCode === 80 && currentGame && currentGame.paused) {
  ...
  update() {
    ...
    if (this.ball.x < 0) {
      ...
      if (this.score2 >= 7) {
        ...
        if (this.wins2 >= 3) this.matchOver = true;
      }
      this.reset();
    }
    if (this.ball.x > 640) {
      ...
      if (this.score1 >= 7) {
        ...
        this.level++;
        this.score1 = this.score2 = 0;
        if (this.wins1 >= 3) this.matchOver = true;
      }
      this.reset();
    }
...
class Squash {
  ...
  update() {
    ...
    const p1 = { x: 580, y: this.p1y, w: 10, h: 66 };
    const p2 = { x: 610, y: this.p2y, w: 10, h: 66 };
    ...
    if (this.collides(b, p1) && this.ball.dx > 0) {
      if (this.lastHitter === 'p2') {
        ...
        this.speedMult *= 1.15;
        this.reset();
        if (this.score1 >= 100) this.gameOver = true;
      } else {
        this.ball.dx *= -1.1;
    ...
    if (this.collides(b, p2) && this.ball.dx > 0) {
      if (this.lastHitter === 'p1') {
        ...
        this.speedMult *= 1.15;
        this.reset();
        if (this.score2 >= 100) this.gameOver = true;
      } else {
        this.ball.dx *= -1.1;
